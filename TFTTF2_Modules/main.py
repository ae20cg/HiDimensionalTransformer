...
#maybe change this to JSON
class DataMapping:
    def __init__(self, col_mappings, batch_size=64,
                 csv_path=None,static_locs=[0,1,2,3,4,5],future_locs = [12,13,14,15,16,17,18,19]):
        ...
        self.static_locs = static_locs
        self.future_locs = future_locs
        self.known_reg_locs = static_locs + future_locs

    def getColMappings(self):
        return self.col_mappings

    def batchData(self):

        if self.data is None:
            return 'There is no data at that location'
        ...
        data_map = {}
        for _, sliced in self.data.groupby(id_col):
    ...
    def getTFData(self):
        ...
        all_inputs = np.concatenate((self.data_map['Known Regular'],self.data_map['Future']),axis=2)
        tf_data = tf.data.Dataset.from_tensor_slices((all_inputs,self.data_map['Target']))

        def make_batches(data):
            return (data.cache().batch(self.batch_size))
...
class HiD_EmbeddingLayer(tf.keras.layers.Layer):
    def __init__(self, time_steps, known_reg_inputs,
                 future_inputs, static_inputs, unknown_len=6, hls=64, cat_inputs=None):  ##hard coded unknown_len to fix
...
class GatedResidualNetwork(tf.keras.layers.Layer):
    ...
    def call(self, inputs, context=None):
        ...
        if self.additional_context is None and context is not None:
            self.additional_context = context
            intermediate = self.linear_additional_context(self.additional_context)
            hidden0 = self.add_layer1([hidden0, intermediate])
...
class TemporalVSN(tf.keras.layers.Layer):
    ...
    def call(self, inputs, context):
        ...
        if self.grn0 is None:
            self.grn0 = GatedResidualNetwork(self.hls, output_size=num_inputs,
                                             dropout_rate=self.dropout_rate,
                                             use_time_distributed=True,
                                             return_gate=True, altered_glu=True)

        mlp_outputs, static_gate = self.grn0(inputs=flatten, context=self.scv)
...
class TemporalFusionTransformer(tf.keras.Model):
    ...
    def __init__(self, num_layers, d_model,
                 num_heads, dff, input_seq_len, output_size,
                 target_seq_len, known_reg_inputs, future_inputs, static_inputs, unknown_inputs=6,
        ...
        self.enriched_grn = None

        self.mlha = InterpretableMultiHeadAttention(self.num_heads, self.hls, dropout=self.dropout_rate)
        ...
        self.self_attn_layer = InterpretableMultiHeadAttention(n_head=self.num_heads,
                                                               d_model=self.d_model, dropout=.1)

        self.mlp = MLP(hls, output_size, output_activation=None, hidden_activation='selu', use_time_distributed=True)
    ...
    def call(self, inputs, training):
        ...
        if self.scvs is None:
            self.scvs = self.static_grn1(static_encoder)
            self.scvs_e = tf.expand_dims(self.scvs, axis=1)
        ...
        historical_features, historical_flags, _ = self.temporal_vsn1(historical_inputs, context=self.scvs_e)
        future_features, future_flags, _ = self.temporal_vsn2(future_inputs, context=self.scvs_e)
        ...
        if self.enriched_grn is None:
            self.enriched_grn = GatedResidualNetwork(hls=self.hls,
                                                     dropout_rate=self.dropout_rate,
                                                     use_time_distributed=True,
                                                     return_gate=True)
        ...
        for FinalGatingLoop in range(0, self.FinalLoopSize):
            ...
            transformer_layer = tf.keras.layers.add([decoder, temporal_feature_layer])
...
#
#     def __init__(self, checkpoint_path,known_reg_locs, future_locs, static_locs):
#
#         self.checkpoint_path = checkpoint_path
#         self.known_reg_locs = known_reg_locs
#         self.static_locs = static_locs
#         self.future_locs = future_locs
#
#         self.model = None
...
#
#         self.model = TemporalFusionTransformer(num_layers=1, d_model=128,
#                  num_heads=4, dff=512, input_seq_len=13, output_size=1,
#                  target_seq_len=15, pe_input=1000, pe_output=1000,
#                  known_reg_inputs=self.known_reg_locs,future_inputs=self.future_locs, static_inputs=self.static_locs,
#                  hls=64,cat_inputs=None,
#                  rate=.1)
#
#     def ckpt_ma
